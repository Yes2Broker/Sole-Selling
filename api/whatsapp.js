 ...
 *   wa_msg         (text)  message body text (must match the approved template)
  ...
  if (req.method === 'GET') {
    ...
    if (selftest) {
      ...
      if (phone.length !== 10 || !slug) {
        res.status(400).json({ ok: false, error: 'use ?selftest=<10-digit>&slug=<project-slug>' });
        return;
      }
      res.status(200).json(await sendForSlug(slug, 'Test', phone));
      return;
    }
    res.status(200).json({
...
async function sendForSlug(slug, name, phone) {
  ...
  try {
    const url = SUPABASE_URL + '/rest/v1/projects?slug=eq.' + slug +
                '&active=eq.true&select=wa_template,wa_msg,wa_media_type,wa_media_url';
